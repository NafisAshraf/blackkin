...
import { SortableProductGrid, type SortableProductItem } from "@/components/admin/SortableProductGrid";
...
function VariantPickerDialog({ open, onClose, forSize, onAdd }: VariantPickerDialogProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={(v) => { if (!v) { onClose(); setSelectedProduct(null); } }}>
          ...
          {!selectedProduct ? (
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" onClick={() => setSelectedProduct(null)} className="h-7 text-xs">
                  Change
                </Button>
              </div>
              {variants === undefined ? (
                ...
                <p className="text-sm text-muted-foreground py-4 text-center">No {forSize} variants for this product.</p>
              ) : (
                <div className="space-y-2 max-h-64 overflow-y-auto">
                  {variants.map((v) => (
                    <div key={v._id} className="flex items-center justify-between p-2.5 border rounded-md">
                      <div>
                      ...
                      >
                        {adding === v._id ? <Loader2 className="h-3 w-3 animate-spin" /> : "Add"}
                      </Button>
                    </div>
                  ))}
                </div>
...
function AlsoBoughtAdminSection() {
  ...
  async function handleAddVariant(variantId: Id<"productVariants">, forSize: string) {
  ...
  const sizeMap = new Map<string, NonNullable<typeof sizeGroups>[number]["items"]>();
  ...
  if (allSizes.length === 0) {
    return <p className="text-sm text-muted-foreground py-4 text-center">Configure sizes in Settings first.</p>;
  }
      ...
      {allSizes.map((size) => {
        ...
        return (
          <div key={size._id} className="space-y-3">
            ...
            {items.length === 0 ? (
              <p className="text-xs text-muted-foreground">No variants added for this size yet.</p>
            ) : (
                ...
                {items.map((item) => (
                  <div key={item._id} className="border rounded-md overflow-hidden group relative">
                    ...
                    <div className="p-2">
                      ...
                      <p className="text-[10px] text-muted-foreground">{item.color ?? "No color"} · {item.stock} in stock</p>
                    </div>
                    <button
                      onClick={() => removeRec({ id: item._id }).then(() => toast.success("Removed")).catch((e: unknown) => toast.error(e instanceof Error ? e.message : "Failed"))}
...
function RecSection({ type, showForSize = false }: { type: RecType; showForSize?: boolean }) {
  ...
  async function handleAdd() {
    ...
    setAdding(true);
    try {
      await addRec({ type, recommendedProductId: selectedProduct.id, forSize: showForSize && forSize !== "_all" ? forSize : undefined });
    ...
    <div className="space-y-3">
      ...
      <div className="flex gap-2 items-end flex-wrap p-3 rounded-md border bg-muted/30">
        <div className="flex-1 min-w-48">
          ...
          <ProductPicker onSelect={setSelectedProduct} placeholder="Search products…" />
        </div>
        {showForSize && (
          <div>
      ...
      {recs === undefined ? (
        ...
        <p className="text-sm text-muted-foreground py-6 text-center">No recommendations yet</p>
      ) : (
        <div className="rounded-md border divide-y">
          <SortableList
            items={recs}
            onReorder={(items) => reorder({ items: items.map(r => ({ id: r.id as Id<"productRecommendations">, sortOrder: r.sortOrder })) }).catch(() => toast.error("Failed to reorder"))}
            renderItem={(rec, dragHandle) => (
              <div className="flex items-center gap-3 px-4 py-3 bg-background">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {rec.forSize && <p className="text-xs text-muted-foreground">Size: {rec.forSize}</p>}
                </div>
                <RowActionsMenu actions={[{ label: "Remove", icon: Trash2, variant: "destructive", onClick: () => removeRec({ id: rec._id }).then(() => toast.success("Removed")).catch(e => toast.error(e.message)) }]} />
              </div>
            )}
          />
        </div>
...
function AdminProductsContent() {
  ...
  const [editCategory, setEditCategory] = useState<{ id: Id<"categories">; name: string } | null>(null);
  const [showNewTag, setShowNewTag] = useState(false);
  const [editTag, setEditTag] = useState<{ id: Id<"tags">; name: string } | null>(null);
  ...
  async function handleBatchAddTag(tagId: string) {
    try {
      if (tagId === "none") {
        await Promise.all([...selectedIds].map((id) => assignTags({ productId: id as Id<"products">, tagIds: [] })));
        ...
        await Promise.all(
          [...selectedIds].map((id) => {
            ...
            if (currentTagIds.includes(tagId as Id<"tags">)) return Promise.resolve();
            return assignTags({ productId: id as Id<"products">, tagIds: [...currentTagIds, tagId as Id<"tags">] });
          })
        );
        toast.success(`Tag added to ${selectedIds.size} product${selectedIds.size !== 1 ? "s" : ""}`);
      }
      clearSelection();
    } catch (e: unknown) {
  ...
  async function handleBatchAddToGroup(groupId: string) {
    try {
      if (groupId === "none") {
        await removeProductsFromAllGroups({ productIds: Array.from(selectedIds) as Id<"products">[] });
        toast.success(`Removed ${selectedIds.size} product(s) from discount groups`);
      } else {
        await addProductsToGroup({ groupId: groupId as Id<"discountGroups">, productIds: Array.from(selectedIds) as Id<"products">[] });
        toast.success(`Added ${selectedIds.size} product${selectedIds.size !== 1 ? "s" : ""} to discount group`);
      }
      clearSelection();
    } catch (e: unknown) {
      toast.error(e instanceof Error ? e.message : "Failed to add to discount group");
    }
  }
  ...
  async function handleBatchDelete() {
    ...
    try {
      await Promise.all([...selectedIds].map((id) => removeProduct({ id: id as Id<"products"> })));
  ...
  function effectiveStatus(p: { status: ProductStatus; scheduledPublishTime?: number }): ProductStatus {
    if (p.status === "scheduled" && p.scheduledPublishTime && p.scheduledPublishTime <= now) return "active";
    return p.status;
  }

  // Group products by effective status
  const nonArchived = (products ?? []).filter((p) => effectiveStatus(p as any) !== "archived");
  const archived = (products ?? []).filter((p) => effectiveStatus(p as any) === "archived");
  const drafts = nonArchived.filter((p) => effectiveStatus(p as any) === "draft").sort((a, b) => a.globalSortOrder - b.globalSortOrder);
  const scheduled = nonArchived.filter((p) => (p as any).status === "scheduled" && (!(p as any).scheduledPublishTime || (p as any).scheduledPublishTime > now)).sort((a, b) => a.globalSortOrder - b.globalSortOrder);
  const active = nonArchived.filter((p) => effectiveStatus(p as any) === "active").sort((a, b) => a.globalSortOrder - b.globalSortOrder);
  ...
  async function handleStatusChange(id: Id<"products">, newStatus: ProductStatus) {
    try {
    ...
    return (
      <TableRow>
        <TableCell className="font-medium">
          <Link href={`/admin/products/${p._id}`} className="hover:underline">{p.name}</Link>
        </TableCell>
        ...
        <TableCell>
          {p.effectivePrice < p.basePrice ? (
            <span className="text-destructive font-medium">৳{p.effectivePrice.toLocaleString()}</span>
          ) : "—"}
        </TableCell>
        <TableCell><Badge variant={badge.variant}>{badge.label}</Badge></TableCell>
        <TableCell className="text-right">
          <RowActionsMenu actions={[
            { label: "Edit", icon: Pencil, onClick: () => router.push(`/admin/products/${p._id}`) },
            {
              label: es === "active" ? "Set to Draft" : "Set to Active",
              icon: es === "active" ? ToggleLeft : ToggleRight,
              separator: true,
              onClick: () => handleStatusChange(p._id, es === "active" ? "draft" : "active"),
            },
            {
              label: "Delete",
              icon: Trash2,
  ...
  function ProductGroupSection({ label, prods, sectionKey, addHref, onReorder, onPublish }: {
    label: string;
    prods: any[];
    sectionKey: string;
    addHref: string;
    ...
    return (
      <div className="space-y-3">
        ...
        >
          ...
          {label}
          <span className="text-xs font-normal ml-1 bg-muted px-1.5 py-0.5 rounded">{prods.length}</span>
        </button>
        {!isCollapsed && (
          viewMode === "grid" ? (
            (() => {
              const items: SortableProductItem[] = prods.map((p) => ({
                ...
                categoryName: categoryMap.get(p.categoryId as Id<"categories">)?.name,
                totalStock: (p.variants ?? []).reduce((sum: number, v: any) => sum + (v.stock ?? 0), 0),
                variantCount: (p.variants ?? []).length,
                tags: (p.tagIds ?? []).map((id: string) => tagMap.get(id as Id<"tags">)?.name).filter(Boolean) as string[],
              }));
              return (
                <SortableProductGrid
                  items={items}
            ...
            <div className="rounded-md border">
              <Table>
                ...
                <TableBody>
                  {prods.length === 0 ? (
                    <TableRow>
                      <TableCell colSpan={6} className="text-center py-6 text-muted-foreground text-sm">No products</TableCell>
                    </TableRow>
  ...
  const isLoading = products === undefined || categories === undefined || tags === undefined;
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button asChild>
            <Link href="/admin/products/new"><Plus className="h-4 w-4 mr-1" />New Product</Link>
          </Button>
        </div>
      </div>
      ...
      {selectedIds.size > 0 && viewMode === "grid" && (
        <div className="flex items-center gap-2 flex-wrap p-3 rounded-lg border bg-background/95 backdrop-blur shadow-md sticky top-4 z-50 text-sm">
          ...
          <Select onValueChange={(groupId) => handleBatchAddToGroup(groupId)}>
            ...
            <SelectContent>
              <SelectItem value="none" className="text-xs text-destructive font-medium focus:text-destructive">No discount group</SelectItem>
          ...
          <Select onValueChange={(tagId) => handleBatchAddTag(tagId)}>
            ...
            <SelectContent>
              <SelectItem value="none" className="text-xs text-destructive font-medium focus:text-destructive">No tag</SelectItem>
              {(tags ?? []).map((t) => (
          ...
          >
            {batchDeleting ? <Loader2 className="h-3.5 w-3.5 animate-spin mr-1" /> : <Trash2 className="h-3.5 w-3.5 mr-1" />}
            Delete
          </Button>
      ...
      <Tabs value={tab} onValueChange={(v) => setTab(v as TabId)}>
        <TabsList className="flex w-full overflow-x-auto">
          ...
          <TabsTrigger value="categories" className="flex-1">
            <FolderOpen className="h-3.5 w-3.5 mr-1.5" />Categories
          </TabsTrigger>
          <TabsTrigger value="tags" className="flex-1">
            <Tags className="h-3.5 w-3.5 mr-1.5" />Tags
          </TabsTrigger>
          <TabsTrigger value="discounts" className="flex-1">
            <Percent className="h-3.5 w-3.5 mr-1.5" />Discounts
          </TabsTrigger>
          <TabsTrigger value="recommendations" className="flex-1">
            <Star className="h-3.5 w-3.5 mr-1.5" />Recommendations
          </TabsTrigger>
        </TabsList>
          ...
          <div className="mt-6 space-y-8">
            {isLoading ? (
              ...
              <>
                {drafts.length > 0 && (
                  <ProductGroupSection
                    label="Drafts" prods={drafts} sectionKey="drafts" addHref="/admin/products/new?from=all"
                    onReorder={(items) => reorderGlobal({ items }).catch(() => toast.error("Failed to reorder"))}
                    onPublish={(id) => handleStatusChange(id, "active")}
                  />
                )}
                {scheduled.length > 0 && (
                  <ProductGroupSection
                    label="Scheduled" prods={scheduled} sectionKey="scheduled" addHref="/admin/products/new?from=all"
                    onReorder={(items) => reorderGlobal({ items }).catch(() => toast.error("Failed to reorder"))}
                  />
                )}
                {active.length > 0 || drafts.length === 0 ? (
                  <ProductGroupSection
                    label="Active" prods={active} sectionKey="active" addHref="/admin/products/new?from=all"
                    onReorder={(items) => reorderGlobal({ items }).catch(() => toast.error("Failed to reorder"))}
                  />
                ) : null}
                {drafts.length === 0 && scheduled.length === 0 && active.length === 0 && (
                  <div className="text-center py-16 text-muted-foreground">
                    <p className="mb-4">No products yet.</p>
                    <Button asChild><Link href="/admin/products/new?from=all"><Plus className="h-4 w-4 mr-1" />Add your first product</Link></Button>
                  </div>
                )}
                ...
                <div className="pt-2 border-t">
                  ...
                  {showArchived && archived.length > 0 && (
                    <div className="mt-3">
                      <ProductGroupSection
                        label="Archived" prods={archived} sectionKey="archived" addHref="/admin/products/new?from=all"
                        onReorder={(items) => reorderGlobal({ items }).catch(() => toast.error("Failed to reorder"))}
                      />
                    </div>
                  )}
        ...
        {tab === "categories" && (
          <div className="mt-6 space-y-6">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">Products organised by category. Drag to reorder within each category.</p>
            ...
            {isLoading ? (
              ...
              <div className="text-center py-12 text-muted-foreground">
                ...
                <Button size="sm" onClick={() => setShowNewCategory(true)}><Plus className="h-4 w-4 mr-1" />Create Category</Button>
              </div>
            ) : (
              <div className="space-y-4">
                {(categories ?? []).sort((a, b) => a.sortOrder - b.sortOrder).map((cat) => {
                  const catProducts = (products ?? [])
                    .filter((p) => p.categoryId === cat._id && effectiveStatus(p as any) !== "archived")
                    .sort((a, b) => a.categorySortOrder - b.categorySortOrder);
                  const isCollapsed = collapsed.has(`cat-${cat._id}`);

                  return (
                    <div key={cat._id} className="rounded-lg border overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-3 bg-muted/30">
                        ...
                        <RowActionsMenu actions={[
                          { label: "Edit", icon: Pencil, onClick: () => setEditCategory({ id: cat._id, name: cat.name }) },
                          {
                            label: cat.isActive ? "Deactivate" : "Activate",
                            icon: cat.isActive ? ToggleLeft : ToggleRight,
                            separator: true,
                            onClick: () => toggleCategoryActive({ id: cat._id, isActive: !cat.isActive }).then(() => toast.success("Category updated")).catch((e) => toast.error(e.message)),
                          },
                          {
                            label: "Delete",
                            icon: Trash2,
                          ...
                          {viewMode === "grid" ? (
                            <SortableProductGrid
                              items={catProducts.map((p) => ({
                                ...
                                imageUrl: p.imageUrl,
                                scheduledPublishTime: (p as any).scheduledPublishTime,
                              ...
                              selectedIds={selectedIds}
                              onToggleSelect={toggleSelect}
                              onReorder={(items) => reorderCategory({ items }).catch(() => toast.error("Failed to reorder"))}
                              prefix={<AddProductCard href={`/admin/products/new?categoryId=${cat._id}&from=categories`} />}
                            />
                          ) : (
                            <div className="rounded-md border">
                              <Table>
                                <TableBody>
                                  {catProducts.length === 0 ? (
                                    <TableRow>
                                      <TableCell className="text-center py-4 text-muted-foreground text-sm">No products in this category</TableCell>
                                    </TableRow>
                                  ) : catProducts.map((p) => <ProductTableRow key={p._id} p={p} />)}
                                </TableBody>
                              </Table>
                            </div>
                          )}
        ...
        {tab === "tags" && (
          <div className="mt-6 space-y-6">
            <div className="flex items-center justify-between">
              <p className="text-sm text-muted-foreground">Products grouped by tag.</p>
            ...
            {isLoading ? (
              ...
              <div className="space-y-4">
                {(() => {
                  const untaggedProducts = (products ?? []).filter(p => (!(p.tagIds as string[]) || (p.tagIds as string[]).length === 0) && effectiveStatus(p as any) !== "archived");
                  if (untaggedProducts.length === 0) return null;
                  return (
                    <div className="mb-6">
                      <ProductGroupSection
                        label="Untagged Products" prods={untaggedProducts} sectionKey="untagged" addHref="/admin/products/new?from=tags"
                      />
                    </div>
                  );
                })()}
                {(tags ?? []).length === 0 ? (
                  <div className="text-center py-12 text-muted-foreground border rounded-lg">
                    ...
                    <Button size="sm" onClick={() => setShowNewTag(true)}><Plus className="h-4 w-4 mr-1" />Create Tag</Button>
                  </div>
                ) : (tags ?? []).map((tag) => {
                  const tagProducts = (products ?? [])
                    .filter((p) => (p.tagIds as string[]).includes(tag._id) && effectiveStatus(p as any) !== "archived")
                    .sort((a, b) => a.globalSortOrder - b.globalSortOrder);
                  const isCollapsed = collapsed.has(`tag-${tag._id}`);

                  return (
                    <div key={tag._id} className="rounded-lg border overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-3 bg-muted/30">
                        <button
                        ...
                        >
                          ...
                          {!tag.isActive && <Badge variant="secondary" className="text-[10px] px-1.5 py-0">Inactive</Badge>}
                          <span className="text-xs text-muted-foreground ml-1">({tagProducts.length})</span>
                        </button>
                        <RowActionsMenu actions={[
                          { label: "Edit", icon: Pencil, onClick: () => setEditTag({ id: tag._id, name: tag.name }) },
                          {
                            label: tag.isActive ? "Deactivate" : "Activate",
                            icon: tag.isActive ? ToggleLeft : ToggleRight,
                            separator: true,
                            onClick: () => toggleTagActive({ id: tag._id, isActive: !tag.isActive }).then(() => toast.success("Tag updated")).catch((e) => toast.error(e.message)),
                          },
                          {
                            label: "Delete",
                            icon: Trash2,
                            ...
                            separator: true,
                            onClick: () => {
                              if (!confirm(`Delete tag "${tag.name}"? Products will not be deleted.`)) return;
                              removeTag({ id: tag._id }).then(() => toast.success("Tag deleted")).catch((e) => toast.error(e.message));
                            },
                          },
                        ]} />
                      </div>
                        ...
                        <div className="p-4">
                          {viewMode === "grid" ? (
                            (() => {
                              ...
                              const productTagIdMap = new Map<string, Id<"productTags">>();
                              for (const p of tagProducts) {
                                const entry = (p.productTagEntries ?? []).find((e: any) => e.tagId === tag._id);
                                if (entry) productTagIdMap.set(p._id, entry.productTagId);
                              }
                              return (
                                <SortableProductGrid
                                  ...
                                  onReorder={(items) => {
                                    const reorderItems = items
                                      .map((item) => {
                                        const productTagId = productTagIdMap.get(item.id);
                                        if (!productTagId) return null;
                                        return { productTagId, sortOrder: item.sortOrder };
                                      })
                                      .filter(Boolean) as { productTagId: Id<"productTags">; sortOrder: number }[];
                                    reorderTag({ items: reorderItems }).catch(() => toast.error("Failed to reorder"));
                                  }}
                                  prefix={<AddProductCard href={`/admin/products/new?tagId=${tag._id}&from=tags`} />}
                                />
                              );
                            })()
                          ) : (
                              ...
                              <Table>
                                <TableBody>
                                  {tagProducts.length === 0 ? (
                                    <TableRow>
                                      <TableCell className="text-center py-4 text-muted-foreground text-sm">No products with this tag</TableCell>
                                    </TableRow>
                                  ) : tagProducts.map((p) => <ProductTableRow key={p._id} p={p} />)}
                                </TableBody>
                              </Table>
                            </div>
                          )}
            ...
            <div className="space-y-4">
              ...
              {discountGroups === undefined ? (
                ...
                <div className="text-center py-10 text-muted-foreground border rounded-lg">
                  ...
                  <Button size="sm" onClick={() => setShowNewGroup(true)}><Plus className="h-4 w-4 mr-1" />Create Group</Button>
                </div>
              ) : (
                <div className="space-y-4">
                      ...
                      <div key={group._id} className="rounded-lg border overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-3 bg-muted/30">
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-xs text-muted-foreground">{groupProducts.length} products</span>
                          </div>
                          <RowActionsMenu actions={[
                            { label: "Edit Group", icon: Pencil, onClick: () => setEditGroup(group) },
                            {
                              label: group.isActive ? "Deactivate" : "Activate",
                              icon: group.isActive ? ToggleLeft : ToggleRight,
                              separator: true,
                              onClick: () => toggleGroupActive({ id: group._id, isActive: !group.isActive }).then(() => toast.success("Updated")).catch((e) => toast.error(e.message)),
                            },
                            {
                              label: "Delete Group",
                              icon: Trash2,
                              ...
                              separator: true,
                              onClick: () => {
                                if (!confirm(`Delete discount group "${group.name}"?`)) return;
                                removeGroup({ id: group._id }).then(() => toast.success("Group deleted")).catch((e) => toast.error(e.message));
                              },
                            },
                          ]} />
                        </div>
                        <div className="p-4 space-y-3">
                          ...
                          <div className="flex gap-2 items-center">
                            <div className="flex-1 max-w-xs">
                              <ProductPicker
                                onSelect={(p) => {
                                  addProductsToGroup({ groupId: group._id, productIds: [p.id] })
                                    .then(() => toast.success(`Added "${p.name}" to group`))
                                    .catch((e) => toast.error(e.message));
                                }}
                                placeholder="Add product to group…"
                              />
                            </div>
                            <Link href={`/admin/products/new?discountGroupId=${group._id}`}>
                              <Button size="sm" variant="outline"><Plus className="h-4 w-4 mr-1" />New Product</Button>
                            </Link>
                          </div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
                              {groupProducts.map((p) => (
                                <div key={p._id} className="relative group">
                                  ...
                                  />
                                  <button
                                    onClick={() => removeProductFromGroup({ groupId: group._id, productId: p._id }).then(() => toast.success("Removed from group")).catch((e) => toast.error(e.message))}
                              ...
                              {groupProducts.length === 0 && <p className="text-sm text-muted-foreground col-span-full py-4">No products in this group yet.</p>}
                            </div>
                          ) : (
                            <div className="rounded-md border">
                              <Table>
                                    ...
                                    <TableRow key={p._id}>
                                      ...
                                      <TableCell className="text-right">
                                        <Button size="sm" variant="ghost" className="text-destructive h-7"
                                          onClick={() => removeProductFromGroup({ groupId: group._id, productId: p._id }).then(() => toast.success("Removed")).catch((e) => toast.error(e.message))}>
            ...
            {products !== undefined && groupMemberships !== undefined && (() => {
              ...
              return (
                <>
                          ...
                          <TableBody>
                            {individualDiscountProducts.map((p) => (
                              <TableRow key={p._id}>
                                ...
                                <TableCell className="text-sm text-muted-foreground">৳{p.discountAmount.toLocaleString()} off</TableCell>
                                <TableCell className="text-right">
                                  <Button size="sm" variant="ghost" asChild><Link href={`/admin/products/${p._id}`}><Pencil className="h-3.5 w-3.5" /></Link></Button>
                                </TableCell>
                              </TableRow>
                            ))}
                          </TableBody>
                  ...
                  <div className="space-y-3">
                    <h2 className="text-base font-semibold text-muted-foreground">Regular Price — No Discount</h2>
                    {noDiscountProducts.length === 0 ? (
                      <p className="text-sm text-muted-foreground">All products have some form of discount.</p>
                    ) : viewMode === "grid" ? (
                      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                      ...
                      <div className="rounded-md border">
                        <Table>
                          <TableBody>
                            {noDiscountProducts.map((p) => <ProductTableRow key={p._id} p={p} />)}
                          </TableBody>
                        </Table>
                      </div>
                    )}
        ...
        {tab === "recommendations" && (
          <div className="mt-6 space-y-6">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">Shown on all product detail pages.</p>
            </div>
            <RecSection type="also_like" />

            <div className="space-y-1 pt-4 border-t">
              <h2 className="text-base font-semibold">People Also Bought</h2>
              <p className="text-xs text-muted-foreground">Shown at checkout, grouped by size. Add specific variants customers should see.</p>
            </div>
            <AlsoBoughtAdminSection />
          </div>
        )}
      ...
      {/* ── Dialogs ── */}
      <QuickCategoryDialog open={showNewCategory || !!editCategory} onOpenChange={(o) => { if (!o) { setShowNewCategory(false); setEditCategory(null); } else setShowNewCategory(true); }} editId={editCategory?.id} editName={editCategory?.name} />
      <QuickTagDialog open={showNewTag || !!editTag} onOpenChange={(o) => { if (!o) { setShowNewTag(false); setEditTag(null); } else setShowNewTag(true); }} editId={editTag?.id} editName={editTag?.name} />
      <DiscountGroupDialog open={showNewGroup || !!editGroup} onOpenChange={(o) => { if (!o) { setShowNewGroup(false); setEditGroup(null); } else setShowNewGroup(true); }} editId={editGroup?._id} editData={editGroup} />
      ...
      <AlertDialog open={!!deleteProductId} onOpenChange={(o) => { if (!o) setDeleteProductId(null); }}>
        <AlertDialogContent>
          ...
          <AlertDialogFooter>
            ...
            <AlertDialogAction onClick={() => deleteProductId && handleDelete(deleteProductId)}>Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
...
export default function AdminProductsPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-24"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-foreground" /></div>}>
