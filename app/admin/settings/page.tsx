    ...
    <DialogContent>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          ...
          <Input id="size-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Small, Medium, XL" required />
        </div>
        <div className="space-y-1">
          <Label htmlFor="size-measurements">Measurements</Label>
          <Textarea id="size-measurements" value={measurements} onChange={(e) => setMeasurements(e.target.value)} placeholder="e.g. Chest: 34-36in, Waist: 28-30in" rows={3} />
        </div>
        <DialogFooter>
...
function SizesTab() {
  ...
  async function handleReorder(reordered: { id: string; sortOrder: number }[]) {
    ...
    setReordering(true);
    try {
      await reorderMutation({ items: reordered.map((r) => ({ id: r.id as Id<"platformSizes">, sortOrder: r.sortOrder })) });
    } catch { toast.error("Failed to reorder"); } finally { setReordering(false); }
  }
    ...
    <div className="space-y-4">
      ...
      {sizes === undefined ? (
        ...
        <div className="rounded-md border divide-y">
          <SortableList items={sizes} onReorder={handleReorder} renderItem={(size, dragHandle) => (
            <div className="flex items-center gap-3 px-4 py-3 bg-background">
              {dragHandle}
              <div className="flex-1 min-w-0">
                <p className="font-medium text-sm">{size.name}</p>
                {size.measurements && <p className="text-xs text-muted-foreground truncate">{size.measurements}</p>}
              </div>
              <RowActionsMenu actions={[
                { label: "Edit", icon: Pencil, onClick: () => setDialogState({ mode: "edit", size }) },
                { label: deletingId === size._id ? "Deleting…" : "Delete", icon: Trash2, variant: "destructive", disabled: deletingId === size._id, separator: true, onClick: () => setDeleteTarget(size) },
              ]} />
            </div>
          )} />
        </div>
    ...
    <DialogContent>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div className="space-y-1">
          ...
          <Input id="color-name" value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g. Midnight Black" required />
        </div>
        <div className="space-y-1">
          <Label htmlFor="color-hex">Hex Code (optional)</Label>
          <div className="flex gap-2 items-center">
            <Input id="color-hex" value={hexCode} onChange={(e) => setHexCode(e.target.value)} placeholder="#000000" />
            {hexCode && <div className="w-8 h-8 rounded border shrink-0" style={{ backgroundColor: hexCode }} />}
          </div>
        </div>
        <DialogFooter>
          <Button type="button" variant="outline" onClick={onClose} disabled={loading}>Cancel</Button>
...
function ColorsTab() {
  ...
  async function handleReorder(reordered: { id: string; sortOrder: number }[]) {
    ...
    setReordering(true);
    try {
      await reorderMutation({ items: reordered.map((r) => ({ id: r.id as Id<"platformColors">, sortOrder: r.sortOrder })) });
    } catch { toast.error("Failed to reorder"); } finally { setReordering(false); }
  }
  ...
  return (
    <div className="space-y-4">
      ...
      {colors === undefined ? (
        ...
        <div className="rounded-md border divide-y">
          <SortableList items={colors} onReorder={handleReorder} renderItem={(color, dragHandle) => (
            <div className="flex items-center gap-3 px-4 py-3 bg-background">
              {dragHandle}
              <div className="flex-1 flex items-center gap-2 min-w-0">
                {color.hexCode
                  ? <span className="h-5 w-5 rounded-full border shrink-0" style={{ backgroundColor: color.hexCode }} />
                  : <span className="h-5 w-5 rounded-full border border-dashed shrink-0 bg-muted" />}
                <div className="min-w-0">
                  <p className="font-medium text-sm">{color.name}</p>
                  {color.hexCode && <p className="text-xs text-muted-foreground">{color.hexCode}</p>}
                </div>
              </div>
              <RowActionsMenu actions={[
                { label: "Edit", icon: Pencil, onClick: () => setDialogState({ mode: "edit", color }) },
                { label: deletingId === color._id ? "Deleting…" : "Delete", icon: Trash2, variant: "destructive", disabled: deletingId === color._id, separator: true, onClick: () => setDeleteTarget(color) },
              ]} />
            </div>
          )} />
        </div>
