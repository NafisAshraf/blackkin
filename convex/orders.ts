...
export const create = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Phase 2 voucher application — re-validates all conditions atomically
...
export const updateStatus = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (admin.role !== "superadmin") {
      if (args.status === "completed" && !admin.permissions?.orders?.canConfirm)
        throw new ConvexError("Unauthorized");
      if (args.status === "deleted" && !admin.permissions?.orders?.canDelete)
        throw new ConvexError("Unauthorized");
    }
    ...
    if (args.status === "cancelled" || args.status === "deleted") {
      const voucherUsage = await ctx.db
        ...
        .withIndex("by_orderId", (q) => q.eq("orderId", args.orderId))
        .unique();
      if (voucherUsage && voucherUsage.status !== "cancelled") {
...
export const createInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Phase 2 voucher application (SSLCommerz: status "pending" until IPN confirms)
    let voucherDiscountAmount = 0;
    if (args.voucherCode && args.voucherCode.trim()) {
      voucherDiscountAmount = await applyVoucherInMutation(ctx, {
        code: args.voucherCode,
...
export const cancelAndRestockInternal = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 3. Release voucher (inline — mutations cannot call other internal mutations)
    const voucherUsage = await ctx.db
      .query("voucherUsages")
      .withIndex("by_orderId", (q) => q.eq("orderId", args.orderId))
      .unique();
...
export const listAllEnriched = query({
  ...
  handler: async (ctx, args) => {
    ...
    const enriched = await Promise.all(
      filteredPage.page.map(async (order) => {
        const customer = await ctx.db.get(order.userId);
        const items = await ctx.db
          .query("orderItems")
...
async function recalcOrderTotals(ctx: MutationCtx, orderId: Id<"orders">) {
  ...
  const total =
    subtotal + (order.deliveryCost ?? 0) - (order.discountAmount ?? 0);
